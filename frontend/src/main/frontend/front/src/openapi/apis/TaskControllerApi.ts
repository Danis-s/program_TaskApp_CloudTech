...
export class TaskControllerApi extends runtime.BaseAPI {
    ...
    /**
     */
    async deleteTaskRaw(requestParameters: DeleteTaskRequest, initOverrides?: RequestInit): Promise<runtime.ApiResponse<string>> {
        if (requestParameters.id === null || requestParameters.id === undefined) {
    ...
    /**
     */
    async deleteTask(id: number, initOverrides?: RequestInit): Promise<string> {
        const response = await this.deleteTaskRaw({ id: id }, initOverrides);
        return await response.value();
    }
